...
c#	[立地点Id] + [立地コスト]
	...
	character*40 Infile(5),Infile2(5)
c#	[需要点のID]+[需要量]
...
c#    距離（評価値）の計算
	Call Dist(mMax,GNum,RList,cost,temp4,LONum,temp6,AList,
     &Gene,DId,DNum,SId,GValue,nMax,SNum,Para,GeneP,LLONum,GeneG,SNum,
     &DNum)
c#
c#    評価値による並び替え
...
c#	[cost2]最適コスト [GeneL]最適配置
	Write(60,*) GValue(1),(Gene(1,i),i=1,LONum)
c#
c#
...
701	continue
	...
	End
c#    ||||||||||||||||||||||||エリート選択||||||||||||||||||||||||||||||
	Subroutine Elie(mMax,nMax,EGene,Gene,ElNum,LONum)
	Integer ElNum,LONum
