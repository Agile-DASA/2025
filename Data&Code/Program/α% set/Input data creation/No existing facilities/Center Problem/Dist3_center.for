...
c#
	...
	Read(*,*) DistMin
c#
	Write(*,*)'最適地点1,最適地点2,最適地点3'
	Read(*,*) point(1),point(2),point(3)
c#
	Write(*,*)'バッファ'
	Read(*,*)bafa
c#
...
c#
		DO 170 j=1,DNum
			If(GValue.LT.RList(j,1))then
      		GValue =  RList(j,1)
			End if
170		continue

		If(GValue.LT.DistMin*1.025)then
		Write(FileNum,*) '[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']', 0
		Else 
      	Write(FileNum,*) '[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']', 1
		End if

		If(GValue.LT.DistMin*1.05)then
		Write(FileNum+1,*) '[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']',0
		Else 
      	Write(FileNum+1,*)'[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']',1
		End if

		If(GValue.LT.DistMin*1.075)then
		Write(FileNum+2,*) '[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']', 0
		Else 
      	Write(FileNum+2,*) '[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']',1
		End if
		
		If(GValue.LT.DistMin*1.1)then
		Write(FileNum+3,*) '[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']', 0
		Else 
      	Write(FileNum+3,*)'[',Area(1,aint),',',Area(2,bint),',',Area(3,cint),']', 1
		End if
